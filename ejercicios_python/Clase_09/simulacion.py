...
class Mundo(object):
    ...
    def etapa_reproduccion(self):
        ...
        for p in self.tablero.posiciones_ocupadas():
            animal = self.tablero.posicion(p)
            animales_cercanos = self.tablero.posiciones_vecinas_con_ocupantes(p)
            # En este punto selecciono solo los animales cercanos que son de 
            #  la misma especie, considerando que dos especies distintas peud
            #  reproducirse, pero no en este mundo.
            if animal.es_antilope:
                animales_cercanos = [vecino[1] for vecino in animales_cercanos if vecino[1].es_antilope()]
            elif animal.leon:
                animales_cercanos = [vecino[1] for vecino in animales_cercanos if vecino[1].es_leon() ]
            posiciones_libres = self.tablero.posiciones_vecinas_libre(p)
            cria = animal.reproducirse(animales_cercanos, posiciones_libres)
            if cria:
                if animal.es_antilope:
                    self.tablero.ubicar_en_posicion_vacia(Antilope())
                elif animal.es_leon:
